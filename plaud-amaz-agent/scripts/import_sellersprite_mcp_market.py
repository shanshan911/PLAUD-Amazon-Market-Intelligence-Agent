...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import SellerSprite MCP market data into PLAUD monitor")
    parser.add_argument("--config", default="config/monitor_config.p0.json", help="Path to monitor config")
    parser.add_argument("--env-file", default=".env.local", help="Optional local env file for MCP secret")
    ...
    parser.add_argument("--node-id-path", default="", help="SellerSprite nodeIdPath; omit to resolve from config")
    parser.add_argument("--month", default="", help="SellerSprite month filter, e.g. 202606")
    parser.add_argument("--top-n", type=int, default=100, help="Top listing count for concentration tools")
    ...
    parser.add_argument("--raw-dir", default="outputs/sellersprite_mcp", help="Directory for raw MCP JSON payloads")
